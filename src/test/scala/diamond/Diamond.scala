...
// Some basic unit test of the Diamond prototype language.
...
class DiamondTest extends AnyFunSuite {
  ...
  test("free var in type") {
    ...
    // Written in let-form so we can be explicit about function's qualifier
    val p2 = """
    val x = Ref 0;
    val f: ((((Int) => Ref[Int]^x)^<>) => Ref[Int]^x)^{x} =
      (g: ((Int) => Ref[Int]^x)^<>): Ref[Int]^x => { g(0) };
    f((y: Int) => { x })
    """
    ...
    topval x = Ref 0;
    def f(g: ((Int) => Ref[Int]^x)^{x, <>}): Ref[Int]^x = g(0);
    val f: ((((Int) => Ref[Int]^x)^{x, <>}) => Ref[Int]^x)^{x} =
      (g: ((Int) => Ref[Int]^x)^{x, <>}): Ref[Int]^x => { g(0) };
    f
    """
    assert(prettyQType(parseAndCheck(p3)) == "(𝑓#3(Arg#4: (𝑓#5(Arg#6: Int^∅) => Ref[Int^∅]^x)^{x,◆}) => Ref[Int^∅]^x)^x")
  }
