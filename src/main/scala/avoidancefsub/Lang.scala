package diamond.avoidancefsub.core
...
enum Expr:
  ...
  // F-sub new expressions
  case ETyLam(f: String, tvar: String, qvar: String, bound: QType, body: Expr, rt: Option[QType], qual: Option[Qual])
...
object ExprSyntax:
  ...
  def Λ(f: String, xt: TypeBound, res: QType)(e: => Expr): ETyLam =
    ETyLam(f, xt.tvar, xt.qvar, xt.bound, e, Some(res), None)
  ...
  def Λ(xt: TypeBound, res: QType)(e: => Expr): ETyLam =
    ETyLam(freshVar("AnnoTFun"), xt.tvar, xt.qvar, xt.bound, e, Some(res), None)
